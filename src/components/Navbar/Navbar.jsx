import React, { useState, useContext } from "react";
import "./Navbar1.css";
import logo from "../../assets/delight_denight_logo.png";
import cart_icon from "../../assets/cart.jpg";
import { Link } from "react-router-dom";
import { ShopContext } from "../../Context/ShopContext";


const Navbar = () => {
  const [menu, setMenu] = useState("home");
  const [isOpen, setIsOpen] = useState(false); // for mobile toggle
  const {getTotalCartItems}=useContext(ShopContext)

  const toggleMenu = () => setIsOpen(!isOpen);

  const handleMenuClick = (menuName) => {
    setMenu(menuName);
    setIsOpen(false); // close menu after click
  };

  return (
    <div className="head">
      <div className="logo_img_name">
        <img src={logo} height="70" alt="Logo" />
        <p>Delight Denights</p>
      </div>

      {/* Hamburger icon for mobile */}
      <div className="hamburger" onClick={toggleMenu}>
        <div className="bar"></div>
        <div className="bar"></div>
        <div className="bar"></div>
      </div>

      <ul className={`nav-items ${isOpen ? "open" : ""}`}>
        <li onClick={() => handleMenuClick("home")}>
          <Link to="/" className="nav-link">HOME</Link>
          {menu === "home" && <hr />}
        </li>
        <li onClick={() => handleMenuClick("vegeterian")}>
          <Link to="/vegeterian" className="nav-link">VEGETERIAN</Link>
          {menu === "vegeterian" && <hr />}
        </li>
        <li onClick={() => handleMenuClick("non-vegeterian")}>
          <Link to="/non-vegeterian" className="nav-link">NON-VEGETERIAN</Link>
          {menu === "non-vegeterian" && <hr />}
        </li>
        <li onClick={() => handleMenuClick("desserts")}>
          <Link to="/desserts" className="nav-link">DESSERTS</Link>
          {menu === "desserts" && <hr />}
        </li>
        <li onClick={() => handleMenuClick("beverages")}>
          <Link to="/beverages" className="nav-link">BEVERAGES</Link>
          {menu === "beverages" && <hr />}
        </li>
        <div className="nav-login-cart">
        <Link to="/login"><button onClick={() => handleMenuClick("login")}>Login</button></Link>
        <div className="carticon"><Link to="/cart"><img  onClick={() => handleMenuClick("cart")} src={cart_icon}  height="40" alt="Cart" /></Link></div>
        <div className="nav-cart-count" onClick={() => handleMenuClick("cart")}>{getTotalCartItems()}</div>
      </div>
      </ul>


    </div>
  );
};

export default Navbar;
